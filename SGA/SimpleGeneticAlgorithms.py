...
class Generation(object):
    ...
    def printResult(self) -> None:
        ...
        print('Coordinates:', self.maxCoordinates[0], self.maxCoordinates[1])
        print()
        df = pd.DataFrame([self.row1, self.row2])
        df.columns = [i for i in range(1,10002)]
        df.index = ['max value', 'average fit']
        df.to_excel('/Users/justin/Desktop/GitHub/Evolutionary-Computing/Simple Genetic Algorithm/data.xlsx')
    ...
    def evaluate(self, loop: int) -> None:
        ...
        for gene in self.population:
            ...
            if gene.eval > self.maxEval[0]:
                self.maxEval = [gene.eval, loop]
                self.maxCoordinates[0] = x
                self.maxCoordinates[1] = y
            if gene.eval > maxValue:
                maxValue = gene.eval
        
        if totalFitness > self.maxTotalFitness:
            self.maxTotalFitness = totalFitness
        
        averageFit = totalFitness / self.popSize
        self.row1.append(maxValue)
        self.row2.append(averageFit)
